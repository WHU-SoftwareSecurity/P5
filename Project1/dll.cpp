...
PLDPartionList _GetDiskPartion() {
	/* 磁盘序号*/
...
vector<UINT64> _GetFileCluster(const char* file_path) {
	char disk_name[7] = "\\\\.\\?:";
...
int _EncodedFileByCluster(const char* file_path, unsigned char* key) {
	char disk_name[7] = "\\\\.\\?:";
	disk_name[4] = file_path[0];
...
int _DecodedFileByCluster(const char* file_path, unsigned char* key) {
	char disk_name[7] = "\\\\.\\?:";
	disk_name[4] = file_path[0];
